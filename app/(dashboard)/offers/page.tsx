...
const mockOffers = Array.from({ length: 50 }, (_, i) => ({
    id: `OFF${i + 1}`,
    image: '/static/images/offers/offer.jpg',
    percentage: 10 + (i % 5) * 5,
            ...
            setTimeout(() => {
                ...
                if (params.search) {
                    filtered = filtered.filter(item =>
                        ...
                        item.percentage.toString().includes(params.search)
                    );
                }

                // Advanced Filters
                if (advancedFilters.percentage) {
                    filtered = filtered.filter(item =>
                        item.percentage.toString().includes(advancedFilters.percentage)
                    );
                }
    ...
    return (
        <Box>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', mb: 2, gap: 2 }}>
                ...
                <Link href="/offers/new" passHref style={{ textDecoration: 'none' }}>
                    <Button
                        variant="contained"
                        startIcon={<AddIcon />}
                        sx={{
