...
const menuItems = [
    ...
    },
    {
        ...
        icon: <LayersIcon />,
        children: [
            { text: 'Sub Category List', path: '/subcategory' },
        ],
    },
    {
        text: 'Products',
    ...
    },
    {
        ...
        icon: <LocalOfferIcon />,
        children: [
            { text: 'Promocode List', path: '/promocode' },
        ],
    },
    {
        text: 'Offers',
...
export default function Sidebar({ open, onToggle }: SidebarProps) { // Modified
    const [openSubmenu, setOpenSubmenu] = React.useState<string | null>(null);
    const pathname = usePathname(); // Get current path

    // State for hover menu in collapsed mode
    const [anchorEl, setAnchorEl] = React.useState<null | HTMLElement>(null); // Added
    const [hoveredMenu, setHoveredMenu] = React.useState<string | null>(null); // Added
    const timeoutRef = React.useRef<NodeJS.Timeout | null>(null); // Added

    const handleClick = (text: string) => {
        if (!open) return; // Don't toggle inline menu when collapsed // Modified
        if (openSubmenu === text) {
            setOpenSubmenu(null);
        } else {
            setOpenSubmenu(text);
    ...
    const handlePopoverOpen = (event: React.MouseEvent<HTMLElement>, text: string) => { // Added
    ...
    const handlePopoverClose = () => { // Added
    ...
    const getActiveMenuItem = (text: string) => menuItems.find(item => item.text === text); // Added

    return (
        <Drawer
            variant="permanent"
            sx={{
                width: open ? drawerWidth : closedDrawerWidth, // Modified
                flexShrink: 0,
                whiteSpace: 'nowrap', // Added
                boxSizing: 'border-box', // Added
                [`& .MuiDrawer-paper`]: {
                    width: open ? drawerWidth : closedDrawerWidth, // Modified
                    transition: (theme) => theme.transitions.create('width', { // Added
        ...
        >
            <Toolbar sx={{
                display: 'flex',
                flexDirection: open ? 'row' : 'column', // Modified
                alignItems: 'center',
                justifyContent: open ? 'space-between' : 'center', // Modified
                px: 2,
                py: 2
            }}>
                {open ? ( // Modified
                                    ...
                                    <Collapse in={open && openSubmenu === item.text} timeout="auto" unmountOnExit>
                                        <List component="div" disablePadding>
                                            {item.children.map((child) => (
                                                <Link key={child.text} href={child.path} passHref >
                                                    <ListItemButton
                                                        sx={{ pl: 9 }}
            ...
            {/* Popover Menu for Collapsed Submenus */} {/* Added */}
            <Popover
