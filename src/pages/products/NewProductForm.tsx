...
import { removeEmptyKeys } from "../../utils/commonHelpers";

// Valid item unit values
const validItemUnits = [
  "LTR",
  "ML",
  "GAL",
  "FL_OZ",
...
const comboDiscountSchema = yup.object().shape({
  comboQuantity: yup
    .number()
    .required("Combo quantity is required")
    .min(1, "Combo quantity must be at least 1")
    .integer("Combo quantity must be a whole number"),
  discountType: yup
    .string()
    .oneOf(["PERCENT", "FLATOFF"], "Discount type must be PERCENT or FLATOFF")
  ...
  discountValue: yup
    ...
    .when("discountType", {
      ...
      otherwise: (schema) =>
        schema.min(0.01, "Flat amount must be greater than 0"),
    }),
  startDate: yup
    .date()
...
const variantSchema = yup.object().shape({
  ...
  itemUnit: yup
    ...
    .oneOf(
      [...validItemUnits],
      "Item unit must be one of the valid unit types",
    ),
  expiryDate: yup
    .date()
...
const imageSchema = yup.object().shape({
  file: yup
    ...
    .test("fileType", "File must be an image", (value) =>
      fileTypeTest(value, true),
    )
    .test("fileSize", "File size must be less than 5MB", (value) =>
      fileSizeTest(value, true),
    ),
...
export default function ProductForm() {
  ...
  const cloneFromProductId = location.state?.cloneFromProductId as
    | number
    | undefined;
  ...
  const [productTypeDialogOpen, setProductTypeDialogOpen] =
  ...
  const [sharedExpiryDate, setSharedExpiryDate] = React.useState<
    Date | string | null
  >(null);
  const [originalVariantIds, setOriginalVariantIds] = React.useState<
    Set<number>
  >(new Set());
  const [originalImageIds, setOriginalImageIds] = React.useState<Set<number>>(
    new Set(),
  );
  ...
  const handleSharedExpiryDateChange = React.useCallback(
    (date: Date | string | null) => {
      ...
      if (useSameExpiryDate && date) {
        ...
        variantFields.forEach((_, index) => {
          setValue(`variants.${index}.expiryDate`, date, {
            shouldValidate: true,
          });
        });
      }
  ...
  const handleUseSameExpiryDateChange = React.useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (checked && sharedExpiryDate) {
        ...
        variantFields.forEach((_, index) => {
          setValue(`variants.${index}.expiryDate`, sharedExpiryDate, {
            shouldValidate: true,
          });
        });
      }
  ...
  React.useEffect(() => {
    ...
    watchedVariants.forEach((variant, index) => {
      ...
      if (units && quantity !== undefined && quantity !== null) {
        ...
        const unitsNum =
          typeof units === "string" ? parseFloat(units.trim()) : units;
        const quantityNum =
        ...
        if (
          ...
          if (variant.itemsPerUnit !== calculatedItemsPerUnit) {
            setValue(`variants.${index}.itemsPerUnit`, calculatedItemsPerUnit, {
          ...
          if (variant.itemsPerUnit !== null) {
            setValue(`variants.${index}.itemsPerUnit`, null, {
              shouldValidate: false,
            });
          }
        }
  ...
  const fetchAndAddItem = React.useCallback(
    async <T extends { id: number }>(
      id: number,
      fetchFn: (params: {
        filters: Array<{ key: string; eq: string }>;
        page: number;
        pageSize: number;
      }) => Promise<{ list: T[] }>,
  ...
  React.useEffect(() => {
    ...
    if (
      ...
      subCategories.some((sub) => sub.categoryId === selectedCategoryId)
    ) {
      return;
    }
    ...
    subCategorySearchTimeoutRef.current = setTimeout(
      async () => {
        try {
    ...
    (async () => {
      ...
      try {
        const response = await getProductTypes({
          filters: [
            { key: "subCategoryId", eq: String(selectedSubCategoryId) },
          ],
          page: 0,
          pageSize: 500,
        });
    ...
    brandSearchTimeoutRef.current = setTimeout(
      async () => {
        ...
        try {
          ...
          if (
            currentBrandId &&
            !newBrands.some((b) => b.id === currentBrandId)
          ) {
  ...
  const buildFormResetData = React.useCallback((data: Product) => {
    ...
    const variants: VariantFormData[] = (data.variants?.map((variant) => {
      ...
      if (variant.expiry_date) {
        expiryDateValue =
          typeof variant.expiry_date === "string"
    ...
    const variantIds = new Set(
      variants.map((v) => v.id).filter((id): id is number => id !== undefined),
    );
  ...
  const buildCloneFormData = React.useCallback(
  ...
  React.useEffect(() => {
    if (!isCloneMode || !cloneFromProductId || hasCloneFetchedRef.current)
      return;
  ...
  React.useEffect(() => {
    if (isEditMode && productData) {
      ...
      const categoryExists =
        ...
        categories.some((cat) => cat.id === productData.category!.id);
      const subCategoryExists =
        !productData.subCategory?.id ||
        subCategories.some((sub) => sub.id === productData.subCategory!.id);
  ...
  const onSubmit = async (data: ProductFormData) => {
    ...
    if (isEditMode) {
      ...
      const hasImageFiles =
        ...
        data.images.some((img) => img.file !== null);
    ...
    try {
      ...
      if (isEditMode) {
        ...
        const variantIdsToDelete = Array.from(originalVariantIds).filter(
        ...
        if (
          originalVariantIds.size > 0 &&
          variantIdsToDelete.length === originalVariantIds.size &&
        ...
        const imageIdsToDelete = Array.from(originalImageIds).filter(
          (id) => !currentImageIds.has(id),
        );
        const reqBody = removeEmptyKeys({
          title: data.title,
          ...
          variantIdsToDelete:
            variantIdsToDelete.length > 0 ? variantIdsToDelete : undefined,
        ...
        await productService.updateProduct(
          id!,
          reqBody as UpdateProductRequest,
        );
        ...
        await productService.createProduct(removeEmptyKeys({
          title: data.title,
          categoryId: data.categoryId,
          subCategoryId: data.subCategoryId,
          productTypeId: data.productTypeId ?? undefined,
            ...
            <Box>
              ...
              >
                {isEditMode
                  ...
                  : isCloneMode
                    ? "Create a copy with the same details (add at least one image)"
        ...
        <Grid container>
                    ...
                    <Box sx={{ flex: 1, minWidth: 0 }}>
                      <FormAutocomplete
                        ...
                        options={[
                          ...
                          !productTypes.some(
                            (pt) => pt.id === productData.productType!.id,
                          )
                            ? [
                                {
                                  value: productData.productType.id,
                                  label: productData.productType.title,
                                },
                              ]
                            : []),
                          ...productTypes.map((pt) => ({
          ...
          <Grid size={{ xs: 12 }}>
            ...
            >
              ...
              >
                Add product variants with different prices, quantities, and
                expiry dates
              </Typography>
                    ...
                    <Box sx={{ flex: 1 }}>
                      <LocalizationProvider dateAdapter={AdapterDateFns}>
                        <DatePicker
                          ...
                          value={
                            sharedExpiryDate
                              ? sharedExpiryDate instanceof Date
                                ? sharedExpiryDate
                                : new Date(sharedExpiryDate)
                              : null
                          }
                          onChange={(newValue) => {
                  ...
                  <Grid container spacing={2}>
                    ...
                    <Grid size={{ xs: 12 }}>
                      <Box sx={{ mt: 2, mb: 2 }}>
                          ...
                          <Button
                            ...
                            onClick={() => {
                              const currentDiscounts =
                                watch(`variants.${index}.comboDiscounts`) || [];
                        ...
                        {watch(`variants.${index}.comboDiscounts`)?.map(
                          (
                            discount: ComboDiscountFormData,
                            ...
                            >
                              ...
                              <Grid container spacing={2}>
                                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                                  ...
                                  >
                                    Exact quantity required (e.g., 2 = buy
                                    exactly 2 items)
                                  </Typography>
                                </Grid>
                                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                                ...
                                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                                  {watch(
                                    `variants.${index}.comboDiscounts.${discountIndex}.discountType`,
                                  ) === "PERCENT" ? (
                                    <FormNumberField
                        ...
                        {(!watch(`variants.${index}.comboDiscounts`) ||
                          watch(`variants.${index}.comboDiscounts`)?.length ===
                            0) && (
                          <Typography
                            variant="body2"
                            sx={{
                          ...
                          >
                            No combo discounts added. Click "Add Combo Discount"
                            to create one.
                          </Typography>
                        )}
                      </Box>
              ...
              <Grid container spacing={2}>
                ...
                {imageFields.length < 3 && (
                  <Grid size={{ xs: 12, md: 4 }}>
                    <Box
                      onClick={() =>
                        !loading && appendImage({ file: null, preview: null })
                      }
                      sx={{
                        border: "1px dashed",
                        borderColor: "divider",
